...
namespace LogicTest
{
    internal class FakeDataBall : IDataBall
    {
        ...
        public FakeDataBall(Vector2 position, Vector2 velocity, int id)
        {
            _position = position;
            _velocity = velocity;
            ID = id;
        ...
        public void SetPositionForTesting(Vector2 newPosition)
        {
    ...
    {
        private readonly List<IDataBall> _balls = new List<IDataBall>();
        ...
        {
            var ball = new FakeDataBall(new Vector2(x, y), new Vector2(vX, vY), id);
            _balls.Add(ball);
            return ball;
        }
