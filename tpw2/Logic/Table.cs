    ...
    internal class Table : LogicAbstractAPI
    {
        ...
        public override void CreateBalls(int n, int r)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                IDataBall dataBall = _dataAPI.CreateDataBall(x, y, r, m, vX, vY);
        ...
        private void CheckCollisionWithWall(Object s, DataEventArgs e)
        {
            ...
            if (!ball.HasCollided)
            {
                if (ball.Position.X + ball.Velocity.X + _ballRadius > _dataAPI.Length ||
                    ball.Position.X + ball.Velocity.X - _ballRadius < 0)
                {
                    ball.Velocity = new Vector2(-ball.Velocity.X, ball.Velocity.Y);
                    ball.HasCollided = true;
                }
                if (ball.Position.Y + ball.Velocity.Y + _ballRadius > _dataAPI.Width ||
                    ball.Position.Y + ball.Velocity.Y - _ballRadius < 0)
                {
                    ball.Velocity = new Vector2(ball.Velocity.X, -ball.Velocity.Y);
                    ball.HasCollided = true;
                }
